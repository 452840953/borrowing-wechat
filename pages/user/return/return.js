...
Page({
  ...
  geto: function (options) {
    var that = this;
    wx.request({
      url: 'http://localhost:8088/book/get2?id=' + options.id + '&sid=' + app.globalData.webUserInfo.id,
      header: {
  ...
  lent: function (e) {
    ...
    wx.request({
      url: 'http://localhost:8088/borrowing/return?stuid=' + stuid + "&bookid=" + bookid + "&status=借阅中",
      header: {
        'content-type': 'application/x-www-form-urlencoded'
      },
      method: 'get',
  ...
  xujie: function (e) {
    ...
    wx.request({
      url: 'http://localhost:8088/borrowing/continue?id=' + id + "&rd=" + that.data.time,
      header: {
        'content-type': 'application/x-www-form-urlencoded'
      },
      method: 'get',
